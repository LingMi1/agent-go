...
本目录是可观测栈的部署产物：单一事实源的 Grafana provisioning（datasource + dashboard）、
...
指标与 trace 互补但不重叠：指标走既有 Prometheus（`internal/metrics` 的 13 个 `myagent_`），
本篇只做 trace，**不碰 OTel metrics**。
...
global-grafana / global-prometheus 在独立 `global-network`。把本目录的 provisioning
...
- **容器**：在 `deploy/Dockerfile.cognition` 的 `uv sync` 加 `--extra otel`（或用构建参数门控），重建镜像。
  暂未默认加入以保持镜像精简（不入核心 deps）；属可选部署步骤。
...
两面日志 `trace_id` 一致。（server-streaming span 可能以 `CANCELLED` 收尾，正常现象。）
