...
默认 Compose 启动 PostgreSQL、Qdrant、Redis 和 MinIO；`--profile app` 再启动内部 cognition 和唯一对外的 control-plane。控制面托管构建后的 SPA，连接 PostgreSQL、MinIO、Qdrant 和 cognition。Tempo 位于额外 `observability` profile。（NATS 已移除：单实例架构不需要事件总线。）
...
Redis 已接线用于 HTTP 限流（固定窗口计数器，fail-open）。认知容器显式使用 local Skill runner，不提供 Docker-in-Docker 沙箱。完整栈不包含 Grafana 或 Prometheus 服务。
